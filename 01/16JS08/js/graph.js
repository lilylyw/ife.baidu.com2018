...
function drawBarChart(data) {
	...
	let unitY_data = all_max / nmuY; //纵轴上，每段的数据长度
	let ratioY = axisY/all_max;      //纵轴上，每单位数据像素数

	let numX = 12;      //横轴均分为12段
	let unitX_px = aisX/numX;  //横轴上，每刻度的像素长度（50px）
	...
	for (let i =1 ;i<=numY;i++) {
		...
		str_textY += '<text x='20' y='${tY}</text>';   //y+3:文字中心与刻度对齐
	};
	...
	let strX = str_scaleX + str_textX + STR_AISx;
    ...
    let color = ["#60acfc","#32d3eb","#5bc49f","#feb64d","#ff7c7c","#9287e7","#9287e7","#60acfc","#32d3eb","#5bc49f"]; //为各个柱子填充不同的颜色
    let len = data.length;
    let str_bar = '';
    let w = unitX_px / (len + 2); //柱子的宽度 = unitX_px / (z柱子总数 + 2)
    for (let i = 0; i < len;i++) {
    ...
    //渲染所有图形元素
    svg.innreHTML = strY + strX + str_baseLine + str_bar;
}

/*绘制折线图，参数data来自于invokeFn()的返回值，支持多数据*/
function drawLineChart(data) {
	//每次调用要先清空原图，重置画布
	let line_wrapper = document.getElementById('line-wrapper');
	line_wrapper.innerHTML = '';
	line_wrapper.innerHTML = '<canvas id="canvas" width="700" height="400"></canvas>';
	...
	for (let i = 0;i < data.length;i++) {
		...
		let t = ['1月','2月','3月','4月','5月','6月','7月','8月','9月','10月','11月','2月'];
		const acisY = 300; //代表Y轴的有效长度，不包含富余，下同
		const axisX = 600;

		let numY = 6;                      //纵轴均分为6段
		let unitY_px = axisY / numY;      //纵轴上，每段的像素长度（50px）
		let unitY_data = all_max / numY; //纵轴上，每单位数据长度
		let ratioY = axisY / all_max;   //纵轴上，每单位数据的像素数

		let numX = 12;
		let unitX_px = axisX / num;   //纵轴上，每单位数据的像素长度（50px）
		...
		//绘制数据点
		let color = ["#60acfc","#32d3eb","#32d3eb","#5bc49f","#feb64d","#ff7c7c","#9287e7","#60acfc","#32d3eb","#5bc49f"];  //为各个数据点、线条填充不同的颜色
	    for (let i = 0;i < data.length;i++) {
	    	let sale = data[i].sale;
	    	 for (let j = 0;j < 12;j++) {
