...
/*Filter data*/
//arr1,the return value of getSelectedCheckbox(regions);
...
function updateTable() {
	let arr1 = getSelectedCheckbox(refions);
...
function uodateGraph() {
	let tbody = document.querySelector('tbody');
	let trList = tbodyquerySelectorAll('tr');
	let data_total = []; //存储表格中的所有销量数据
	for (let i = 0; i < triList.length;i++) {
		let tdList = triList[i].querySeletorAll('td');
