...
let selectedDiv = document.getElmentById('selected');
...
function selectValue(dom) {
	...
	for (let i = 0;i < checkArray.length - 1;i++) {
		if (checkArray[i].checked == true) {
			data.push(checkedArray[i].value);
		}
	}
	return data;
}
...
function render() {
	...
	let value = hrefValue() || '';
	if (value === '') {
		regionLength = selecteValue(regionSelect).length;
	...
	for (let i = 0;i < data.length;i++) {
		//类型与地区
		if (i % rowSpan === 0) {
			text += '<tr td-sign=' + data[i][txt1[1]] + '><td rowspan=' + rowSpan + '>' + data[i][txt1[1]] + '</td><td td-sign=' + data[i][txt2[1]] + '>' + data[i][txt2[1]] + '</td>';	
		}else {
...
function setHash() {
	...
	let products = selectValue(productSelect);
	let regionHash = '';
	let productHash = '';
	//分隔符
	...
	for (let i = 0;i < productS.length;i++) {
		productHash += encodeURI(producS[i]);
	}
